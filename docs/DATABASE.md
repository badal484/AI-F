...
## Current schema (through Phase 12 — Billing)
...
- `getPlatformDb()` bypasses tenant scoping entirely. It exists for exactly five legitimate uses: (1) resolving which tenant a bare Supabase session belongs to, before a `tenantId` exists to scope with; (2) the Platform Admin Dashboard (Phase 13); (3), added Phase 8, resolving which tenant an inbound WhatsApp webhook is for (`apps/web/src/app/api/webhooks/whatsapp/route.ts`) by its `phone_number_id` — a webhook has no Supabase session either, and here it's the `X-Hub-Signature-256` check (not a session) that makes trusting the payload's claimed `phone_number_id` safe; (4), added Phase 10, resolving which tenant a website widget message is for (`apps/web/src/app/api/widget/[tenantId]/message/route.ts`) by the `tenantId` in its URL path — a widget visitor has no session either, and here it's the `Origin` header check against that tenant's own `widgetAllowedOrigins` that makes trusting it safe; and (5), added Phase 12, resolving which tenant a Stripe webhook event is for (`apps/web/src/app/api/webhooks/stripe/route.ts`) by the Stripe customer id on the event — a Stripe webhook has no session either, and here it's Stripe's own signature check that makes trusting the event's claimed customer id safe. In every case, `getPlatformDb()` is used only for that initial identity resolution — the actual read/write work that follows switches to `getTenantDb(tenantId)`. Its name and import path are deliberately distinct from `getTenantDb()` so any cross-tenant access is visually obvious in a diff.
