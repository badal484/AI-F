...
### Anthropic prompt caching in `draftReply()` (targeting Phase 5 / Phase 16)

`packages/ai/src/reply.ts`'s `draftReply()` now marks its system prompt as an Anthropic cache breakpoint. The system content moved from the separate `system` string param into `messages[0]` as an explicit `SystemModelMessage` (`{ role: "system", content, providerOptions: { anthropic: { cacheControl: { type: "ephemeral" } } } }`) — a plain string param has no way to carry `providerOptions`, and cache control is a per-message field. Required `allowSystemInMessages: true` on the `generateText()` call, since this `ai@7.0.68` defaults that to `false` and throws if a `role: "system"` message appears in `messages` without it (confirmed in the SDK's own runtime source, not assumed).

**A field-name correction, not just an implementation detail:** the originally-requested field (`experimental_providerMetadata`) doesn't exist as a message-level input in this installed SDK version — `providerMetadata` is a *result* field returned by the SDK, not something you set going in. The actual mechanism, confirmed by reading `@ai-sdk/anthropic`'s compiled runtime source (not the docs, not memory), is `providerOptions.anthropic.cacheControl`. Using the requested field name would have compiled (or been silently dropped) without ever actually enabling caching — same "verify against the installed SDK's real shape" discipline this repo has followed since Phase 5 (`inputSchema` not `parameters`, `stopWhen: stepCountIs()` not `maxSteps`) and reapplied here.

**Honest scope note on actual impact right now:** this system prompt is currently well under Anthropic's minimum cacheable length for Claude Sonnet models — 1,024 tokens (confirmed against current Anthropic docs). Below that threshold, `cache_control` is accepted but has no effect — the mechanism is correctly wired, but there's nothing to point to yet showing a real latency/cost win, because the cached block itself is too small to actually cache. Business hours and RAG chunks were **not** added to this cached block to reach that threshold, despite being named in the original task — this codebase's actual architecture fetches both on demand via tool calls (`getBusinessInfo`, `searchKnowledgeBase`), not by pre-stuffing them into the prompt; RAG retrieval in particular has no query to search with until the model is already mid-turn, so there's nothing to pre-fetch into a static block ahead of time. Changing that would be a real architecture change, not a caching patch, and wasn't attempted here. The mechanism is ready for whenever a large enough static block exists ahead of the dynamic conversation.

Verified: `npm run typecheck`/`lint`/`build` clean across all 12 workspaces. The sentiment/language analysis logic added in Phase 16 (`analyzeMessage`, `writeMessageAnalysis`) and all five tool configurations were not touched. Not verified: an actual cache hit against the real Anthropic API — no `ANTHROPIC_API_KEY` configured in this environment, the same standing gap as every AI-dependent feature across this whole build.

### First real live-infrastructure run — diagnostic audit, two bugs found, seed script (2026-08-20)

For the first time since Phase 0, this app ran against a real Postgres, real Redis, and a real Supabase Auth project, with an actual person clicking through the browser — not just typecheck/lint/build against nothing. That surfaced two genuine bugs no amount of static verification could have caught, alongside a clean result on everything auditable statically.

**Environment note (not app code):** the local machine's Postgres 16 had no working `pgvector` build — Homebrew's `pgvector` bottle only ships extension files for Postgres 17/18. Rather than touch the existing v16 install, local dev now runs a second Postgres 17 instance (port 5434, its own data directory) with `pgvector` 0.8.6 installed and confirmed working; v16 is untouched. This is a local-machine detail, not a schema or code change — `packages/db/prisma/schema.prisma`'s `extensions = [vector]` / `postgresqlExtensions` preview feature needed no changes and is confirmed correctly formatted for Prisma 7 by this same real migration succeeding end-to-end (`prisma migrate dev`, 26 tables, `CREATE EXTENSION vector` included in the generated SQL).

**Static audit — clean:**
- No `TODO`/`FIXME`/`XXX` markers anywhere in `apps`/`packages` source.
- Every `@aif/*` import across all 12 workspaces cross-checked against that importing package's own `package.json` dependencies — zero missing declarations.
- `TENANT_SCOPED_MODELS` in `packages/db/src/tenant.ts` cross-checked field-by-field against every model in `schema.prisma` carrying a `tenantId` column — exact match, no gaps. Confirmed (re-reading the extension) that a model in neither `TENANT_SCOPED_MODELS` nor `SELF_SCOPED_MODELS` still throws rather than passing through unscoped.
- No code outside `packages/db` itself calls `getBasePrisma()` directly (the one way tenant isolation could be bypassed) — grepped across both apps. All three BullMQ queue processors in `apps/worker/src/queues/` go through `getTenantDb(tenantId)`, never a bare client.
- `npm run typecheck`, `npm run lint`, `npm run build` — all clean, exit 0, across all 12 workspaces, both before and after this patch's own changes.

**Bug #1 — dev-mode CSP blocked React's own debugging (`apps/web/next.config.ts`):** Phase 14's production CSP (`script-src 'self' 'unsafe-inline'`, deliberately no `unsafe-eval`) was applied identically in development, but Next 16/React's dev-mode tooling (HMR, stack-trace reconstruction) itself calls `eval()` — the strict CSP blocked it, and Next's dev error overlay surfaced the resulting console error as a blocking "Console Error" screen even though it's functionally non-fatal (React's own message: "will never use eval() in production mode"). Fixed by adding `'unsafe-eval'` to `script-src` **only when `NODE_ENV === "development"`** — the production policy Phase 14 built is untouched.

**Bug #2 — retrying signup after an unconfirmed email crashed instead of explaining (`apps/web/src/domains/auth/actions.ts`'s `signUp()`):** Supabase deliberately returns success (same user id, no error) when `auth.signUp()` is called again for an email with an existing account — even an unconfirmed one — to avoid leaking which emails are registered. `signUp()` didn't account for this: it unconditionally tried `tx.user.create()` afterward, hitting the `supabaseUserId` unique constraint and falling into the generic `"Account created but workspace setup failed. Contact support."` catch-all instead of a real answer. Fixed by checking for an existing `User` row keyed on `data.user.id` before the transaction, returning "An account with this email already exists. Check your inbox to confirm it, or sign in instead." when found.

**New: `packages/db/prisma/seed.ts` (`npm run db:seed` from the repo root, or `npm run seed -w @aif/db`).** A local/staging provisioning script, entirely idempotent (every write is an `upsert`/find-or-create keyed on a real unique constraint — safe to re-run any number of times, confirmed by running it twice and diffing row counts/ids, which came back identical). Provisions:
- One `Agency` (`demo-agency`) and one `Tenant` (`demo-tenant`) linked to it.
- The Tenant's primary `Location`, default Mon–Fri 9am–5pm `LocationHours` (closed weekends), one `Service`, and one `StaffMember`.
- One `PlatformAdmin` — **only** if `SEED_PLATFORM_ADMIN_SUPABASE_USER_ID`/`SEED_PLATFORM_ADMIN_EMAIL` are set. Unlike everything else the script creates, a `PlatformAdmin` must reference a real Supabase auth user id (same rule as `User.supabaseUserId`), so it genuinely can't be fabricated — per MASTER_INSTRUCTIONS.md §7 ("No fake implementations... log as NOT CONFIGURED"), this step is skipped with a clear message rather than inventing a placeholder id. Both new env vars documented in `.env.example`.
- Wired up via the standard `"prisma": { "seed": "..." }` field in `packages/db/package.json` (so `prisma migrate dev`'s own seed prompt also finds it), plus a plain `seed`/`db:seed` npm script. Added `tsx` as a `packages/db` devDependency to run it (already used elsewhere in the monorepo, e.g. `apps/worker`, just not previously a dependency of `packages/db` itself). `packages/db/tsconfig.json`'s `rootDir` widened from `"src"` to `"."` (`outDir`/emit are unused by this package regardless — no package in this repo has an actual `tsc` build step; every package is consumed as TypeScript source via `apps/web`'s `transpilePackages`) so `prisma/seed.ts` type-checks under `npm run typecheck` rather than living outside the checked tree.

**Live verification commands** (local dev, once `DATABASE_URL`/`REDIS_URL` point at a reachable Postgres+`pgvector`/Redis):
```bash
npm install
npm run db:generate               # regenerate the Prisma client
npx prisma migrate dev --name init -w @aif/db   # or: cd packages/db && npx prisma migrate dev --name init
npm run db:seed                   # provisions Agency/Tenant/Location/Service/StaffMember
npm run dev:web                   # apps/web, terminal 1
npm run dev:worker                # apps/worker, terminal 2
```

**Noted, not fixed — pre-existing, unrelated to this patch:** `npm audit` reports 3 high-severity advisories, all a single transitive chain — `prisma@7.9.1` → `@prisma/config` → `deepmerge-ts` (stack exhaustion on recursive object graphs, GHSA-ggr8-5vv4-36mx). The only fix `npm audit fix --force` offers is downgrading to `prisma@6.12.0` — a real breaking change (undoing this repo's Prisma 7 migration) not made unilaterally here; flagged for a deliberate decision instead.

Verified: `npm run typecheck`/`lint`/`build` clean across all 12 workspaces after every change in this entry (both the two bug fixes and the new seed script/config changes). The seed script itself was run twice against the real local database described above and confirmed idempotent. Not verified: `PlatformAdmin` seeding (no real Supabase auth user id was created and supplied for this specific check), and the two bug fixes' real-world trigger conditions were both actually reproduced against real Supabase Auth + real Postgres in this environment (not just reasoned through) — see the "Bug #1"/"Bug #2" entries above for the exact server-log evidence each was caught from.
